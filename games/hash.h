...
public:
    hashtable()
    {
        if (ck_ht_init(&ht, CK_HT_MODE_BYTESTRING | CK_HT_WORKLOAD_DELETE, NULL, &ck_allocator, 10, 42) == false) {
    ...
    bool set(KEY* key, VALUE* value)
    {
        ...
        ck_ht_entry_t entry;
        ck_ht_hash_t h;
        ck_ht_hash(&h, &ht, (void*)key, (uint16_t)key_size);
    ...
    bool get(KEY* key, VALUE** value)
    {
        ...
        ck_ht_hash_t h;
        ck_ht_hash(&h, &ht, (void*)key, (uint16_t)key_size);
        ck_ht_entry_key_set(&entry, (void*)key, key_size);
    ...
    bool remove(KEY* key) {
        ck_ht_entry_t entry;
        ck_ht_hash_t h;
        ck_ht_hash(&h, &ht, (void*)key, key_size);
        ck_ht_entry_key_set(&entry, (void*)key, key_size);
